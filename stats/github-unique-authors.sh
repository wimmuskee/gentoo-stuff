...
for line in $(cat /tmp/gitlogs.csv); do
	...
	yearmonth=${date:0:7}
	echo "${yearmonth},${email}" >> ${GITLOGS_PARSED}
done
...
for yearmonth in $(cat ${GITLOGS_PARSED} | cut -d "," -f 1 | sort -r | uniq); do
	...
	total=$(cat ${GITLOGS_PARSED} | grep ${yearmonth} | cut -d "," -f 2 | sort | uniq | wc -l)
	gentoo=$(cat ${GITLOGS_PARSED} | grep ${yearmonth} | grep "gentoo.org" | cut -d "," -f 2 | sort | uniq | wc -l)
	other=$(($total - $gentoo))
	echo "${yearmonth},${total},${other}" >> ${PLOT_DATA}
done

# plot
gnuplot -e "plot_data='${PLOT_DATA}'" github-unique-authors.plt
